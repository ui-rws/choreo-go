...
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'tlCKbJjSfrVytzZrf4';
...
app.get('/sub', (req, res) => {
  ...
  const vlessURL = `vless://${UUID}@${CFIP}:433?encryption=none&security=tls&sni=${ARGO_DOMAIN}&type=ws&host=${ARGO_DOMAIN}&path=%2Fvless?ed=2048#${NAME}-${ISP}`;
  const vmessURL = `vmess://${Buffer.from(JSON.stringify(VMESS)).toString('base64')}`;
  const trojanURL = `trojan://${UUID}@${CFIP}:433?security=tls&sni=${ARGO_DOMAIN}&type=ws&host=${ARGO_DOMAIN}&path=%2Ftrojan?ed=2048#${NAME}-${ISP}`;
